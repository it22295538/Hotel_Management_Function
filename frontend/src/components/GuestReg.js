...
export default function GuestRegister({ guest }) {
  ...
  useEffect(() => {
    if (guest) {
      ...
      setNic(guest.nic || '');
      setPassportNum(guest.passportNum || '');
      setAddress(guest.address || '');
      setChildren(guest.children || '');
      setRoomDetails(guest.roomDetails || '');
  ...
  function handleCountryChange(event) {
    ...
    setNic(selectedCountry === 'Sri Lanka' ? '' : nic);
    setPassportNum(selectedCountry !== 'Sri Lanka' ? '' : passportNum);
    ...
    try {
      ...
      if (country === 'Sri Lanka') {
        if (!(/^\d{9}([Vv])?$/.test(nic) || /^\d{12}$/.test(nic))) {
          throw new Error('Invalid NIC format. It should be either a 12-digit number or a 9-digit number followed by "V" or "v".');
        }
      } else {
        if (!passportNum) {
      ...
      const url = isUpdate
      ? `${ApiURL}/Hotel/Guest/update/${guest._id}`
      : `${ApiURL}/Hotel/Guest/register`;
  ...
  return (
    <div className="container mt-5">
      <h2>{isUpdate ? 'Update Guest' : 'Guest Registration Form'}</h2>
      ...
      <form onSubmit={handleRegistration}>
        ...
        <div className="mb-3">
          ...
          <select id="country" className="form-select" value={country} onChange={handleCountryChange} required>
        ...
        {country === 'Sri Lanka' ? (
          ...
          <div className="mb-3">
            <label htmlFor="passportNum" className="form-label">Passport Number</label>
            <input type="text" className="form-control" id="passportNum" value={passportNum} onChange={(e) => setPassportNum(e.target.value)} required />
          </div>
        )}
        <div className="mb-3">
        ...
        <div className="mb-3">
          ...
          <input type="number" className="form-control" id="children" value={children} onChange={(e) => setChildren(e.target.value)} required />
        </div>
        <div className="mb-3">
          <label htmlFor="roomDetails" className="form-label">Room Details</label>
          <textarea className="form-control" id="roomDetails" value={roomDetails} onChange={(e) => setRoomDetails(e.target.value)} required />
        </div>
        <div className="mb-3">
          <label htmlFor="chckin" className="form-label">Check-in Date</label>
          <input type='date' className="form-control" id="checkin" value={checkin} onChange={(e) => setCheckin(e.target.value)} required />
        </div>
        <div className="mb-3">
          <label htmlFor="checkout" className="form-label">Checkout Date</label>
